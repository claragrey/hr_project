...
from hr_app1.forms import ResumeUploadForm, AnswerForm
...
def generate_closing(model, previous_questions=[]):
    ...
    - "Thank you for your insightful answers, [Candidate Name]. We appreciate your time."
...
def get_overall_feedback(ratings_explanations):
        ...
        try:
            ...
            return average_ratings_html, error_message, final_rating # Return HTML for error
...
def index(request, interview_type=None):
    ...
    return render(request, 'hr_app1/index.html', {'form': form, 'interview_type': interview_type})
...
def question(request, interview_type):
    ...
    return render(request, 'hr_app1/question.html', {
...
def results(request):
        ...
        overall_feedback_text_lines = BeautifulSoup(overall_feedback_text, 'html.parser').get_text(strip=True).split('\n')
        overall_feedback_text = "<strong>Overall Feedback:</strong><ul>"
        for line in overall_feedback_text_lines:
            if line.strip():  # Ensure no empty lines are added
                overall_feedback_text += f"<li>{line.strip()}</li>"
        overall_feedback_text += "</ul>"
        ...
        return render(request, 'hr_app1/results.html', context)

def home(request):
        return render(request, 'hr_app1/home.html')

def ai_interview_options(request):
        return render(request, 'hr_app1/ai_interview_options.html')
